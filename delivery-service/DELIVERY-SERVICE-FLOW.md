...
| `fcm` | `PUSH_PROVIDER=fcm` | Android FCM HTTP v1 API 호출 |
...
Android FCM 실제 외부 발송 검증은 Firebase project id, service account 인증 정보, Android FCM registration token이 필요합니다. `.env.local`에 `PUSH_PROVIDER=fcm`, `FCM_PROJECT_ID`, `GOOGLE_APPLICATION_CREDENTIALS` 또는 `FCM_CREDENTIALS_JSON`, `FCM_TITLE`을 설정한 뒤 실행합니다.
