...
SMS와 PUSH는 채널별 sender 경계가 분리되어 있으며, 현재는 logging Provider로 동작합니다. SMS는 Twilio 설정을 환경변수로 받을 수 있지만, Twilio 실제 발송은 이후 Phase에서 추가합니다. FCM 실제 발송도 이후 Phase에서 추가합니다.
...
| `TWILIO_ACCOUNT_SID` | 빈 값 | Twilio Account SID. `twilio` provider에서 필수 예정 |
| `TWILIO_AUTH_TOKEN` | 빈 값 | Twilio Auth Token. `twilio` provider에서 필수 예정 |
| `TWILIO_FROM_NUMBER` | 빈 값 | Twilio 발신 번호. Messaging Service 미사용 시 필수 예정 |
| `TWILIO_MESSAGING_SERVICE_SID` | 빈 값 | Twilio Messaging Service SID. 발신 번호 대신 사용 가능 예정 |
| `TWILIO_API_URL` | `https://api.twilio.com/2010-04-01` | Twilio API URL |

현재 Phase에서는 설정 바인딩만 추가합니다. 실제 Twilio API 호출과 필수값 검증은 Twilio SMS sender 구현 Phase에서 추가합니다.
