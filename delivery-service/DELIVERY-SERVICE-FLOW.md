...
PUSH는 채널별 sender 경계가 분리되어 있으며, 현재는 logging Provider로 동작합니다. FCM 설정은 환경변수로 받을 수 있지만, FCM 실제 발송은 이후 Phase에서 추가합니다.
...
| `FCM_PROJECT_ID` | 빈 값 | Firebase project id. `fcm` provider에서 필수 예정 |
...
현재 Phase에서는 설정 바인딩만 추가합니다. 실제 FCM API 호출, service account 인증, 필수값 검증은 Android FCM sender 구현 Phase에서 추가합니다.

PUSH 1차 구현에서는 `NotificationEvent.recipient`를 Android FCM registration token으로 해석합니다.
...
Android FCM 실제 외부 발송 검증은 Firebase project id, service account 인증 정보, Android FCM registration token이 필요합니다. 실제 발송 절차는 Android FCM sender 구현 Phase에서 문서화합니다.
