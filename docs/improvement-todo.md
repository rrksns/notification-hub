...
**작성일**: 2026-03-23
**최종 수정**: 2026-08-21 (내부 서비스 JWT 필터 적용)
**기준**: 실무 관점 코드 리뷰 결과
...
| 25 | 전체 | SecurityConfig `permitAll()` | user, notification, delivery, analytics 서비스 API가 공통 Servlet JWT 필터를 통과해야 접근 가능. 공개 경로는 user 등록/인증과 actuator health로 제한됨 | K8s NetworkPolicy 추가 필요 | 진행 중 |
