// user-service SecurityConfig의 공개 경로와 보호 경로 인증 정책을 검증하는 테스트
...
@TestPropertySource(properties = {
        "jwt.security.excluded-paths=/api/users/register,/api/auth/**,/actuator/health,/actuator/health/**"
})
class SecurityConfigTest {
    ...
    @Test
    @DisplayName("API key 보호 경로는 토큰 없이 401을 반환한다")
    void apiKeys_withoutToken_returnsUnauthorized() throws Exception {
    ...
    @Test
    @DisplayName("회원 등록 경로는 토큰 없이 접근할 수 있다")
    void register_withoutToken_isPublic() throws Exception {
        mockMvc.perform(post("/api/users/register"))
                .andExpect(status().isOk());
    }

    @Test
    @DisplayName("로그인 경로는 토큰 없이 접근할 수 있다")
    void login_withoutToken_isPublic() throws Exception {
    ...
    void apiKeys_withValidToken_usesTrustedTenantHeader() throws Exception {
    ...
    static class SecurityProbeController {
        ...
        @PostMapping("/api/keys/security-probe")
        String protectedApiKey(@RequestHeader("X-Tenant-Id") String tenantId) {
            return tenantId;
        }
